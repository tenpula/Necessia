...
export const SidebarRight: React.FC<SidebarRightProps> = ({ paper }) => {
  return (
    <aside className="w-96 flex-none bg-white dark:bg-[#111118]/95 backdrop-blur-xl border-l border-slate-200 dark:border-[#282839] z-30 flex flex-col h-full overflow-y-auto transform transition-transform duration-300 absolute right-0 top-0 bottom-0 shadow-2xl">
      <div 
      ...
      <div className="flex flex-col p-6 gap-6 -mt-12 relative">
        <div className="flex flex-col gap-3">
          ...
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white leading-tight font-display">
            {paper.title}
          </h2>
          ...
          <div className="flex flex-wrap gap-2">
            {paper.authors.map((author, idx) => (
              <div key={idx} className="flex items-center gap-2 bg-slate-100 dark:bg-[#282839] rounded-full pr-3 pl-1 py-1">
                <div className="size-6 rounded-full bg-slate-300 dark:bg-[#3b3b54] flex items-center justify-center text-[10px] font-bold text-white">{author.initials}</div>
                <span className="text-xs font-medium text-slate-700 dark:text-white">{author.name}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-2">
            {paper.connectedNodes.map((node) => (
              <a key={node.id} href="#" className="group flex items-center gap-3 p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-[#282839] transition-colors">
                <div className="size-8 rounded bg-primary/20 text-primary flex items-center justify-center flex-none">
                  <span className="material-symbols-outlined text-[18px]">article</span>
                </div>
                <div className="flex flex-col min-w-0">
                  <p className="text-sm font-medium text-slate-900 dark:text-white truncate group-hover:text-primary transition-colors">{node.title}</p>
                  <p className="text-xs text-slate-500 dark:text-[#9d9db9]">{node.year} • {node.authors}</p>
                </div>
              </a>
            ))}
          </div>
